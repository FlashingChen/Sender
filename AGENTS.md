...
## 项目一句话
自托管个人通知采集系统：安卓端采集通知（发送者+内容+时间）→ Go 服务端存储 → Web 控制台与 OAuth API 查询。
...
- 环境变量：`ADDR`（默认 `:8080`）、`DB_PATH`、`TZ`（默认 `Asia/Shanghai`）、`ALLOW_REGISTRATION`（注册完自己的账号后设 `false`）。
...
## 目录与约定
- `server/` 服务端；`android/` 采集端；`design/` Web UI 设计定稿（direction-approved.md + draft-2-terminal.html）；`tasks/` 任务书；`data/` 生产数据库。
